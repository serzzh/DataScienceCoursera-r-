...
confusionMatrix(pretest$classe, pretest1)
```
Slow. Out-of-sample accuracy is around 80%.
...
We have found that could work well on our data. 
...
Finally we are using both random forest and LDA algorithm with 4-fold cross validation 

LDA algorithm

```{r}
ldaFit <- train(train.feat, training$classe, method="lda", trControl = trainControl(method = 'cv', number = 8))
saveRDS(ldaFit, "lda.RDS")
## ldaFit<-readRDS('lda.RDS')
```
...
## LDA
test1 <- predict(ldaFit, newdata=test.feat)
confusionMatrix(testing$classe,test1)
##Random Forest
test2 <- predict(rfFit, newdata=test.feat)
confusionMatrix(testing$classe,test2)
```

So after expanding our training data and using cross-validation random forest gives us perfect results while LDA stucks at 71% accuracy.
